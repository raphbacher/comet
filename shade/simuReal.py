...
class SourceSim():
    ...
    def __init__(self,shape=(41,101,101),lmbda=20,noise=None, \
        ...
        decrease='True',noiseCube=None,df_stu=5,randomShape=False,seed=None,steps=50):
        ...
        if randomShape==False:
            ...
            self.maskSources,self.label=buildRandomMaskSources(shape,self.listCenter,self.listRadius,seed=seed,steps=steps)
            np.random.seed()
        self.spectraSources=[]
...
def buildRandomMaskSources(shape=(40,51,51),listCenter=[(25,25)],listRadius=[15],steps=50,seed=False):
    center=listCenter[0]    
    if seed is not None:
        np.random.seed(seed)
